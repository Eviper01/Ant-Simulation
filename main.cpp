...
#define Trail_Diffuse                                                          \
  0.005 // percent after decay that spreads to the adjacenet nodes
...
int renderAndupdate_field(double *Field_Intensity, double *Field_Maturity,
...
int scan_field_SIMD(struct ant_struct *Ant, double *Field, double *Angle) {
  double max_intensity = Trail_Maturity;
  int Detected = 0;
  for (int Row = -Ant_Sense_Range; Row <= Ant_Sense_Range; Row++) {
    // This is repeated with Col -Ant_Sense to Ant_Sense
    //
    for (int Col = -Ant_Sense_Range; Col <= Ant_Sense_Range; Col++) {
      ...
      if (Index > 0) {
        ...
        float DeltaX = (float)Col;
        float DeltaY = (float)Row;
        int MagnitudeSquared = pow(Col, 2) + pow(Row, 2);

        double *intensity = (Field + Index);
        int Suitable = (MagnitudeSquared > 1.0 && *intensity > max_intensity &&
                        *intensity > Ant->max_intensity);

        *Angle = atan2(DeltaY, DeltaX) * Suitable + *Angle * (!Suitable);
        Detected = Suitable || Detected;
        max_intensity = *intensity * Suitable + max_intensity * (!Suitable);
        Ant->max_intensity =
            *intensity * Suitable + (Ant->max_intensity) * (!Suitable);

      } else {
        Col += -Index;
      }
      if (Index > Canvas_X * Canvas_Y) {
        break;
      }
    }
  }
...
int ant_update(struct ant_struct *Ant, struct food_struct **Foods_List,
               ...
               double *Foraging_Field_Maturity, double *Homing_Field_Intensity,
               double *Homing_Field_Maturity,
               struct colony_struct *Colonys_List) {
  // TODO: refactor to avoid recyled code
  Ant->max_intensity +=
      Trail_Aging; // the algoarimt for this needs to prevent ants getting stuck
...
void loop(sf::RenderWindow *window, struct ant_struct **Ants_List,
          double *Foraging_Field_Intensity, double *Foraging_Field_Maturity,
          double *Homing_Field_Intensity, double *Homing_Field_Maturity,
...
int main() {
  ...
  double *Foraging_Field_Intensity =
      (double *)malloc(Canvas_X * Canvas_X * sizeof(double));
  double *Homing_Field_Intensity =
      (double *)malloc(Canvas_X * Canvas_X * sizeof(double));
  double *Foraging_Field_Maturity =
      (double *)malloc(Canvas_X * Canvas_X * sizeof(double));
  double *Homing_Field_Maturity =
      (double *)malloc(Canvas_X * Canvas_X * sizeof(double));
